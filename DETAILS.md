...
One codebase serving several static photo/video galleries on Cloudflare Workers
(static assets). Today: [booperandwoowoo.com](https://booperandwoowoo.com) and
[jeremyandtipsy.com](https://jeremyandtipsy.com).

Each site is its own Worker with its own R2 bucket, custom domain and content, built from
this one repository. Everything that differs lives in `sites/<domain>/site.json`; see
[README.md](README.md) for the layout and the `GALLERY_SITE` convention.
...
```bash
npm install
GALLERY_SITE=booperandwoowoo.com npm run dev
```

## Deploy

Each site is a separate Worker connected to this same repository, distinguished by its
**Root directory**. In the Cloudflare dashboard (**Workers & Pages** → the Worker →
**Settings** → **Builds** → **Connect**):

| Setting | Value |
| --- | --- |
| Root directory | `sites/<domain>` |
| Build command | `cd ../.. && npm ci && npm run build <domain>` |
| Deploy command | `npx wrangler deploy` |

The Worker's name in the dashboard must match `name` in that directory's `wrangler.jsonc`,
or the build is rejected. Set **Build watch paths** to include `app/`, `scripts/` and that
site's own directory, so a change to one gallery's photos doesn't rebuild the other.

Manual deploy, if you ever need it:

```bash
npx wrangler login
GALLERY_SITE=<domain> npm run deploy
```

## Custom domain

Attached via `routes` in each site's `wrangler.jsonc` (`custom_domain: true`). Requires
that domain to already be an active zone on this Cloudflare account, and no conflicting
DNS record on the root hostname.

## Adding photos and videos

- Drop new photos into `sites/<site>/public/pictures/`.
- Drop new videos into `sites/<site>/public/videos/` (this folder is git-ignored — the files themselves
...
- Run:
  ...
  This updates `sites/<site>/public/gallery.json` and generates everything it needs. It only ever
  ...
  - **Local video backup** (via `scripts/download-videos.mjs`, `npm run downloadVideos`):
    ...
    `sites/<site>/public/videos/` under its expected name. Purely peace-of-mind — nothing in the
    ...
    video.
  - **Photos**: a thumbnail in `sites/<site>/public/thumbs/` (via `scripts/generate-thumbnails.mjs`,
    using `sharp`).
  - **Videos** (via `scripts/process-videos.mjs`, using `ffmpeg-static`/`ffprobe-static`):
    a poster-frame thumbnail in `sites/<site>/public/thumbs/`; a small muted ~3s hover-preview clip; and
    ...
    at all). The preview clip and full video are uploaded to the site's R2
    R2 bucket, keyed by the video's own poster name (`videos/<name>.mp4`,
    `previews/<name>.mp4`), and served from that site's `mediaBaseUrl`; only the
  ...
  - Safe to re-run: unchanged videos are skipped (tracked in the git-ignored
    ...
    the whole batch.
  - Requires `npx wrangler login` once, with access to the site's R2
    bucket.
- Commit `sites/<site>/public/pictures/`, `sites/<site>/public/thumbs/`, `sites/<site>/public/gallery.json` and
  `media-sequence.json`. Do **not** commit `sites/<site>/public/videos/` — it's ignored on purpose.

## File naming

Anything dropped into `sites/<site>/public/pictures/` or `sites/<site>/public/videos/` is renamed on the next
...
```
sites/<site>/public/pictures/WhatsApp Image 2026-08-09 at 12.00.00.jpeg  ->  000000000000.jpg
sites/<site>/public/videos/PXL_20260809_120001.MP4                       ->  000000000001.mp4
```
...
`sites/<site>/public/gallery.json` is the **source of truth**, not the filesystem. That matters because
the two media types are backed very differently:

| | Photos | Videos |
|---|---|---|
| Originals live in | `sites/<site>/public/pictures/` — **committed** | `sites/<site>/public/videos/` — **git-ignored** |
| Served from | this Worker's static assets | R2, via the site's `mediaBaseUrl` |
...
For videos this is a hard requirement — `sites/<site>/public/videos/` is git-ignored, so a fresh clone
...
differently (rebuilt fresh from `sites/<site>/public/pictures/` every run, so a file going missing —
...
```powershell
npm run deletePicture booper-in-a-hat.jpg yes
npm run deleteVideo woowoowiggles.mp4 yes
```
...
be the source filename or the bare stem (`woowoowiggles`). An ambiguous picture stem
(`booper.jpg` *and* `booper.png` both existing) is refused rather than guessed at.

**Photos** — `npm run deletePicture <name> yes` deletes the original from
`sites/<site>/public/pictures/`, the thumbnail from `sites/<site>/public/thumbs/`, and the entry from
...
- removes the entry from `sites/<site>/public/gallery.json`
- deletes the poster JPG from `sites/<site>/public/thumbs/`
- deletes the source from `sites/<site>/public/videos/`, if this machine has it
...
Then commit `sites/<site>/public/gallery.json` and the deleted poster.
...
```powershell
curl -s -o /dev/null -w "%{http_code}" "https://media.booperandwoowoo.com/videos/<name>.mp4?cb=$(Get-Random)"
```
...
dashboard → the `booperandwoowoo.com` zone → **Caching** → **Configuration** →
...
"full": "https://media.booperandwoowoo.com/videos/000000000047.mp4"
```

the two commands are:

```powershell
npx wrangler r2 object delete booperandwoowoo-media/previews/000000000047.mp4 --remote
npx wrangler r2 object delete booperandwoowoo-media/videos/000000000047.mp4 --remote
```
