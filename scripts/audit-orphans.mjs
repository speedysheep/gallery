// Read-only report of mismatches between sites/<site>/public/gallery.json and the files on disk.
...
//   - orphaned thumbnail/poster: a file in sites/<site>/public/thumbs/ that no gallery.json entry
...
//     sites/<site>/public/pictures/ file is gone, or a video whose poster is gone) — the entry is
...
async function main() {
	...
	if (orphanedThumbs.length > 0) {
		console.log(`${orphanedThumbs.length} orphaned thumbnail(s) in sites/<site>/public/thumbs/ (not referenced by any gallery.json entry):`);
	...
	if (missingPhotoFiles.length > 0) {
		console.log(`${missingPhotoFiles.length} photo entr${missingPhotoFiles.length === 1 ? "y" : "ies"} whose file is missing from sites/<site>/public/pictures/:`);
	...
	if (missingPosterFiles.length > 0) {
		console.log(`${missingPosterFiles.length} video entr${missingPosterFiles.length === 1 ? "y" : "ies"} whose poster is missing from sites/<site>/public/thumbs/:`);
