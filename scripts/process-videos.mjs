// Processes every video in sites/<site>/public/videos/ and publishes it to R2:
//   - a poster JPG (first frame) into sites/<site>/public/thumbs/ — committed to git, just like photo thumbs
...
// Entries are merged into sites/<site>/public/gallery.json (type: "video"), alongside whatever
...
// sites/<site>/public/videos/ is git-ignored: on a fresh clone that folder is empty, and anything
...
//
// Requires: `npx wrangler login` once, with access to the selected site's R2 bucket, and
// GALLERY_SITE set to the site you mean (see scripts/lib/site.mjs).
...
export function scaledDims(width, height, longEdge) {
...
export function needsReencode(info) {
	return (
		info.codec !== "h264" ||
...
export async function makePoster(srcPath, outJpgPath, info) {
	await mkdir(stagingDir, { recursive: true });
	const framePath = path.join(stagingDir, `poster-${path.basename(outJpgPath)}.raw.jpg`);
	const at = Math.min(1, info.duration / 2);
	// Anamorphic video (non-square pixels) decodes to the wrong shape unless the pixel
	// aspect ratio is applied: a clip stored 1280x960 with a 9:16 SAR is meant to be shown
	// 720x960. Players honour SAR on their own, but a still frame does not, which is how
	// posters ended up stretched. iw/ih are the dimensions ffmpeg *actually decoded*, so
	// this stays correct for rotated sources too (ffmpeg auto-applies the display matrix
	// before filters, and hardcoding numbers from ffprobe's coded dims would fight it).
	// setsar=1 then leaves a plain square-pixel JPEG everything downstream agrees on.
	await run(
		["-y", "-ss", String(at), "-i", srcPath, "-frames:v", "1", "-vf", "scale=iw*sar:ih,setsar=1", "-q:v", "2", framePath],
		"poster extraction",
	);
	// The raw frame is the source's true display size; the caller uses it as the shape of
	// record, so the manifest can never disagree with the poster the viewer is looking at.
...
async function quarantine(name, reason) {
	...
	try {
		...
		console.warn(`    (couldn't move it: ${err.message}; leaving it in sites/<site>/public/videos/)`);
	}
}
...
async function main() {
	...
	console.log(`Found ${videos.length} videos in sites/<site>/public/videos/, ${existingVideos.length} already in gallery.json.`);
	...
	for (const name of videos) {
		...
		try {
			...
			const posterDims = await makePoster(srcPath, posterPath, info);
		...
		// Display shape, not coded dims: posterDims came off the decoded frame, so it already
		// accounts for pixel aspect ratio and rotation. needsReencode() still asks the coded
		// question — that one is about encoding cost, not about shape.
		const finalDims = needsReencode(info) ? scaledDims(posterDims.width, posterDims.height, MAX_LONG_EDGE) : posterDims;
	...
	if (carriedOver > 0) {
		console.log(`${carriedOver} entries have no source file in sites/<site>/public/videos/ and were left untouched (that folder is git-ignored).`);
	}
