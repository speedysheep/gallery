...
//
//   npm run deletePicture <name>        e.g. npm run deletePicture booper-in-a-hat.jpg
//   npm run deleteVideo   <name>        e.g. npm run deleteVideo   woowoowiggles.mp4
//
...
// enough ("booper" finds "booper.jpg"); ambiguous stems (booper.jpg *and* booper.png)
...
async function resolvePicture(arg) {
	...
	if (byStem.length > 1) {
		console.error(`"${arg}" is ambiguous — sites/<site>/public/pictures/ has ${byStem.join(", ")}.`);
...
/** Accepts "clip.mp4", "clip.MOV", "clip.jpg", "sites/<site>/public/videos/clip.mp4" or "clip". */
...
async function runPicture(name, { dryRun, real }) {
	...
	if (!resolved) {
		console.error(`No photo in sites/<site>/public/pictures/ matches "${name}".`);
		...
		if (near.length) {
			...
			console.error(`\nsites/<site>/public/pictures/ has ${candidates.length} photos.`);
		}
		process.exit(1);
	}
	...
	if (real) {
		console.log("  removed the entry from sites/<site>/public/gallery.json");
		console.log("\nDone. Commit sites/<site>/public/gallery.json and the deleted pictures/ and thumbs/ files.");
	} else {
		console.log("  would remove the entry from sites/<site>/public/gallery.json");
	}
}
...
async function runVideo(name, { dryRun, real }) {
	...
	if (real) {
		...
		console.log("\nDone. Commit sites/<site>/public/gallery.json and the deleted thumbs/ poster.");
	} else {
		console.log("  would remove the entry from sites/<site>/public/gallery.json");
...
async function main() {
	...
	if (!kind || !["picture", "video"].includes(kind) || !name) {
		...
		console.error("   e.g. npm run deletePicture booper-in-a-hat.jpg");
		console.error("        npm run deleteVideo woowoowiggles.mp4");
		process.exit(1);
	}
