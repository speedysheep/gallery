...
//
// Photos: every JPEG in sites/<site>/public/pictures/ is rewritten in place with the same lossless
...
async function backfillPhotos(dryRun) {
	...
	console.log(`\n=== Photos: ${files.length} in sites/<site>/public/pictures/ ===`);
...
async function main() {
	...
	if (!dryRun) {
		console.log("\nDone.");
		if (doPhotos) console.log("Commit the rewritten files in sites/<site>/public/pictures/.");
