...
//
// The single most important rule this module encodes: sites/<site>/public/gallery.json is the
...
import { displayDims } from "./geometry.mjs";
import { SITE, repoRoot, siteConfig, siteDir } from "./site.mjs";

export const execFileAsync = promisify(execFile);

// Every path below is scoped to the one site chosen by GALLERY_SITE — see lib/site.mjs.
// `repoRoot` remains the repository root; anything site-owned hangs off `siteDir`.
export { SITE, siteConfig, siteDir, repoRoot };
...
export const cachePath = path.join(siteDir, ".media-cache.json");
// Committed, unlike .media-cache.json — the counter has to survive a clone, or a second
// machine would restart numbering and collide with names already in use. It is per-site:
// two galleries numbering independently is fine, sharing a counter is not.
export const sequencePath = path.join(siteDir, "media-sequence.json");
...
export const R2_BUCKET = siteConfig.r2Bucket;
export const R2_PUBLIC_BASE_URL = siteConfig.mediaBaseUrl;

export const VIDEO_EXTENSIONS = [".mp4", ".mov", ".m4v"];

// Lives in geometry.mjs so the unit tests can reach it without choosing a site.
export { displayDims };
...
/**
 * Reads sites/<site>/public/gallery.json. Returns null (not []) when it can't be read or parsed,
