// Generates gallery thumbnails for every photo in sites/<site>/public/pictures/.
// Run with: node scripts/generate-thumbnails.mjs (or `npm run gallery` to also process videos)
// Run any time you add photos to sites/<site>/public/pictures/ before deploying.
//
// ADD-ONLY, like scripts/process-videos.mjs: a photo already in gallery.json keeps its
// entry even if its file later goes missing from sites/<site>/public/pictures/ (accidental delete, bad
...
async function main() {
	...
	console.log(`Found ${photos.length} photos in sites/<site>/public/pictures/, ${existingPhotos.length} already in gallery.json.`);
	...
	if (carriedOver > 0) {
		console.log(`${carriedOver} entries have no file in sites/<site>/public/pictures/ — run \`npm run audit\` for details.`);
	}
