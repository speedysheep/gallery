...
//
// Requires: `npx wrangler login`, with access to the selected site's R2 bucket, and
...
async function phase0Preflight() {
	...
	console.log(`Backed up sites/<site>/public/gallery.json -> ${path.relative(repoRoot, backupPath)}`);
 ...
 * covers every JPEG in sites/<site>/public/pictures/ regardless of name. */
...
async function phase5VideoRekey(manifest, dryRun, counterRef) {
	...
	for (const entry of videos) {
		...
		// Local source, if this machine happens to have it (rare — sites/<site>/public/videos/ is
...
async function main() {
	...
	if (!manifest) {
		console.error("Couldn't read sites/<site>/public/gallery.json — refusing to touch anything.");
		process.exit(1);
	}
